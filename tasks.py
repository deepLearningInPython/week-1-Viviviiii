...
# a numpy array, and an integer (call argument "cutoff" and set default to 0).
# The function should return a numpy array of the same length,
# with all elements smaller than the cutoff being set to cutoff.
...
def ReLu(array, cutoff=0):
    return [max(x, cutoff) for x in array]
...
# Task 3:
# Instructions:
# Write a function that takes in a two-dimensional numpy array of size (n, p)
# and a one-dimensional numpy array of size p.
